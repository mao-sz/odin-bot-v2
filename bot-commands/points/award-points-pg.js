...
function getAwards(content, author) {
  ...
  let shouldNotifyAboutDoublePoints = false;

  for (const [_, userID, awardType] of content.matchAll(awardPointsRegex)) {
    if (awardType === '?++') {
      if (isAdmin(author)) {
        awards.set(userID, 2);
      } else if (!shouldNotifyAboutDoublePoints) {
        shouldNotifyAboutDoublePoints = true;
      }
  ...
  return [awards, shouldNotifyAboutDoublePoints];
}
  ...
  cb: async function pointsBotCommand({
    ...
    try {
      ...
      const { rows: upsertedUsers } = await db.query(
        `
          INSERT INTO points(discord_id, points)
